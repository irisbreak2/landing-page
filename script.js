document.addEventListener("DOMContentLoaded", () => {
  ...
  if (brokerCard && registerContainer && !document.getElementById("broker-overview")) {
  ...
  if (toolShowcase) {
    ...
    const toolPreview = {
      ...
      media: toolShowcase.querySelector(".tool-choice-preview__media"),
      image: toolShowcase.querySelector("[data-tool-preview-image]"),
      imageSecondary: toolShowcase.querySelector("[data-tool-preview-image-secondary]"),
    ...
    const activateToolOption = (button, shouldMovePreview = false) => {
      ...
      if (toolPreview.media && toolPreview.imageSecondary) {
        const hasSecondaryImage = Boolean(data.imageSecondary);
        toolPreview.media.classList.toggle("is-split", hasSecondaryImage);
        toolPreview.imageSecondary.hidden = !hasSecondaryImage;
        if (hasSecondaryImage) {
          toolPreview.imageSecondary.src = data.imageSecondary;
          toolPreview.imageSecondary.alt = data.altSecondary || "ตัวอย่างเครื่องมือ TTS ใน Algo Prime";
        } else {
          toolPreview.imageSecondary.removeAttribute("src");
          toolPreview.imageSecondary.alt = "";
        }
      }
  ...
  const instructorProofImages = [
    "assets/private-class/proof/earth-funded-certificates.webp",
    "assets/private-class/proof/earth-profit-split-01.webp",
    "assets/private-class/proof/earth-profit-split-02.webp",
    "assets/private-class/proof/earth-profit-split-03.webp"
  ];

  const classPosterImages = [
    "assets/private-class/private-class-main-poster-v8.webp",
    "assets/private-class/class-support-poster.webp",
    "assets/private-class/course-syllabus-poster.webp"
  ];
  ...
  document.querySelectorAll("[data-instructor-proof-index]").forEach((button) => {
    button.addEventListener("click", () => openImageLightbox({
      images: instructorProofImages,
      title: "ใบรับรองและประสบการณ์ของ Earth Tatchai",
      index: Number(button.dataset.instructorProofIndex)
    }));
  });

  document.querySelectorAll("[data-class-poster-index]").forEach((button) => {
    button.addEventListener("click", () => openImageLightbox({
      images: classPosterImages,
      title: "โปสเตอร์รายละเอียด Private Class",
      index: Number(button.dataset.classPosterIndex)
    }));
  });
  ...
  const normalizePage = (href) => {
    if (!href || href.startsWith("#")) return currentPage;
    try {
  ...
  navLinks.forEach((link) => {
    ...
    link.classList.toggle("is-current", normalizePage(href) === currentPage);
  });
    ...
    const sectionObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        navLinks.forEach((link) => {
          ...
          const isSameSection = linkPage === currentPage && linkHash === `#${entry.target.id}`;
          const isSamePageWithoutSection = linkPage === currentPage && !linkHash;
          link.classList.toggle("is-current", isSameSection || isSamePageWithoutSection);
        });
      });
